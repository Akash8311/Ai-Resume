...
/* ═══════════════════════════════════════════
   ─── LETTER ANIMATION ENGINE ───
   Every single letter in every heading/text
   gets a scroll-triggered staggered reveal
   ═══════════════════════════════════════════ */
function SplitText({
  text,
  as: Tag = "span",
  ...
  delay = 0,
  stagger = 28,
  type = "slideUp",      // slideUp | slideDown | fadeScale | rotateIn | wave | glitch | blur | flipX | dropIn
  wordByWord = false,
  className = "",
  color,
}) {
  ...
  const units = wordByWord
    ? text.split(" ").map((w, i, arr) => (i < arr.length - 1 ? w + " " : w))
    : text.split("");

  const getTransform = (i) => {
    const baseDelay = `${delay + i * stagger}ms`;
    const hidden = {
      slideUp:   "translateY(110%) skewY(4deg)",
      slideDown: "translateY(-110%) skewY(-4deg)",
      fadeScale: "scale(0.4) translateY(30px)",
      rotateIn:  "rotateX(-90deg) translateY(20px)",
      wave:      `translateY(${Math.sin(i * 0.8) * 40 + 60}px)`,
      glitch:    `translateX(${i % 2 === 0 ? -20 : 20}px) skewX(${i % 2 === 0 ? 10 : -10}deg)`,
      blur:      "translateY(20px) scale(1.1)",
      flipX:     "rotateY(-90deg) translateX(-20px)",
      dropIn:    "translateY(-80px) rotate(-8deg)",
    }[type] || "translateY(110%)";

    const visibleTransform = type === "wave"
      ? "translateY(0px)"
      : "translateY(0) translateX(0) scale(1) rotate(0) skewY(0) skewX(0) rotateX(0) rotateY(0)";

    return {
      display: "inline-block",
      opacity: visible ? 1 : 0,
      transform: visible ? visibleTransform : hidden,
      filter: type === "blur" ? (visible ? "blur(0px)" : "blur(8px)") : "none",
      transition: `opacity 0.65s cubic-bezier(.22,1,.36,1) ${baseDelay},
                   transform 0.65s cubic-bezier(.22,1,.36,1) ${baseDelay},
                   filter 0.5s ease ${baseDelay}`,
      ...charStyle,
    };
  };

  return (
    <Tag ref={ref} style={{ overflow: "hidden", display: "inline", ...style }} className={className}>
      {units.map((char, i) => (
        <span
          key={i}
          style={{
            ...getTransform(i),
            color: color ? (typeof color === "function" ? color(i, units.length) : color) : undefined,
...
/* Word-line variant: wraps each word so overflow:hidden clips each word */
function SplitWords({ text, as: Tag = "div", style = {}, wordStyle = {}, delay = 0, stagger = 60, type = "slideUp", className = "" }) {
  const ref = useRef();
  const [visible, setVisible] = useState(false);
  ...
  return (
    <Tag ref={ref} style={{ display: "flex", flexWrap: "wrap", gap: "0 0.25em", ...style }} className={className}>
      {words.map((word, i) => (
        <span key={i} style={{ overflow: "hidden", display: "inline-block" }}>
          <span style={{
            ...
            transform: visible
              ...
              : type === "slideUp" ? "translateY(110%)" : "translateY(-110%)",
            transition: `opacity 0.7s cubic-bezier(.22,1,.36,1) ${delay + i * stagger}ms,
                         transform 0.7s cubic-bezier(.22,1,.36,1) ${delay + i * stagger}ms`,
            ...wordStyle,
          }}>
            {word}
          </span>
...
function WelcomeScreen({ onDone }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i <= 20; i++) {
        ...
        ctx.moveTo(0, yPos); ctx.lineTo(W, yPos); ctx.stroke();
      }
      ...
      for (let i = 0; i <= 20; i++) {
        ...
        ctx.lineTo(baseX, H); ctx.stroke();
      }
      particles.forEach(p => {
        p.z += p.vz;
        if (p.z < 1) { p.z = 1000; p.x = Math.random() * W; p.y = Math.random() * H; p.trail = []; }
        const scale = 800 / p.z;
        ...
        if (p.trail.length > 1) {
          ...
          ctx.strokeStyle = grad; ctx.lineWidth = r * 0.8; ctx.stroke();
        }
        if (sx > -10 && sx < W + 10 && sy > -10 && sy < H + 10) {
          ctx.beginPath(); ctx.arc(sx, sy, Math.max(0.5, r), 0, Math.PI * 2);
          ctx.fillStyle = `hsla(${p.hue},90%,65%,${alpha})`; ctx.fill();
        }
      });
      ...
      grad.addColorStop(0.4, `rgba(22,163,74,${0.06})`);
  ...
  useEffect(() => {
    ...
    const glitchIv = setInterval(() => { setGlitch(true); setTimeout(() => setGlitch(false), 120); }, 700);
    const t1 = setTimeout(() => setPhase("out"), 2500);
    const t2 = setTimeout(() => onDone(), 3100);
    return () => { clearTimeout(t1); clearTimeout(t2); clearInterval(iv); clearInterval(glitchIv); };
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "3px", background: "linear-gradient(90deg,transparent,rgba(34,197,94,0.8),transparent)", animation: "scanSweep 2.4s ease-in-out infinite", boxShadow: "0 0 20px rgba(34,197,94,0.9)", zIndex: 2 }} />
      {[{ top: 20, left: 20, borderTop: "2px solid rgba(34,197,94,0.6)", borderLeft: "2px solid rgba(34,197,94,0.6)" }, { top: 20, right: 20, borderTop: "2px solid rgba(34,197,94,0.6)", borderRight: "2px solid rgba(34,197,94,0.6)" }, { bottom: 20, left: 20, borderBottom: "2px solid rgba(34,197,94,0.6)", borderLeft: "2px solid rgba(34,197,94,0.6)" }, { bottom: 20, right: 20, borderBottom: "2px solid rgba(34,197,94,0.6)", borderRight: "2px solid rgba(34,197,94,0.6)" }].map((s, i) => (
        <div key={i} style={{ position: "absolute", width: 40, height: 40, ...s, opacity: 0.7, zIndex: 3, animation: `cornerPulse 2s ease-in-out ${i * 0.3}s infinite alternate` }} />
      ))}
      <div style={{ position: "absolute", top: 60, left: 60, zIndex: 3, fontFamily: "'Fira Code',monospace", fontSize: 11, color: "rgba(34,197,94,0.45)", lineHeight: 1.8 }}>
      ...
      <div style={{ position: "absolute", top: 60, right: 60, zIndex: 3, fontFamily: "'Fira Code',monospace", fontSize: 11, color: "rgba(34,197,94,0.45)", lineHeight: 1.8, textAlign: "right" }}>
      ...
      <div style={{ position: "relative", zIndex: 4, textAlign: "center" }}>
        <div style={{ position: "relative", width: 110, height: 110, margin: "0 auto 36px", animation: "logoBounce 1s cubic-bezier(.34,1.56,.64,1) both" }}>
          <div style={{ position: "absolute", inset: -16, borderRadius: "50%", border: "1px solid rgba(34,197,94,0.3)", animation: "spinCW 6s linear infinite" }} />
          <div style={{ position: "absolute", inset: -28, borderRadius: "50%", border: "1px dashed rgba(34,197,94,0.15)", animation: "spinCCW 10s linear infinite" }} />
          <div style={{ position: "relative", width: 110, height: 110, borderRadius: 28, background: "rgba(130,201,158,0.95)", border: "1.5px solid rgb(34,197,94)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 52, boxShadow: "0 0 40px rgba(34,197,94,0.5)" }}>🤖</div>
        </div>
        <div style={{ position: "relative", marginBottom: 8 }}>
          {glitch && (
            <>
              <h1 style={{ fontSize: "clamp(40px,7vw,80px)", fontWeight: 900, letterSpacing: "-0.06em", color: "rgba(34,197,94,0.8)", fontFamily: "Georgia,serif", position: "absolute", top: 0, left: 0, right: 0, transform: "translateX(-3px)", clipPath: "inset(20% 0 40% 0)" }}>AI<span style={{ color: "#4ade80" }}>Resume</span></h1>
              <h1 style={{ fontSize: "clamp(40px,7vw,80px)", fontWeight: 900, letterSpacing: "-0.06em", color: "rgba(34,197,94,0.8)", fontFamily: "Georgia,serif", position: "absolute", top: 0, left: 0, right: 0, transform: "translateX(3px)", clipPath: "inset(60% 0 10% 0)" }}>AI<span style={{ color: "#4ade80" }}>Resume</span></h1>
            </>
          )}
          <h1 style={{ fontSize: "clamp(40px,7vw,80px)", fontWeight: 900, letterSpacing: "-0.06em", color: "#fff", fontFamily: "Georgia,serif", margin: 0, animation: "fadeUp 1s cubic-bezier(.22,1,.36,1) .3s both", textShadow: "0 0 60px rgba(34,197,94,0.4)" }}>
            AI<span style={{ color: G[400] }}>Resume</span>
          </h1>
        </div>
        <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 13, letterSpacing: ".22em", textTransform: "uppercase", fontWeight: 600, fontFamily: "'Fira Code',monospace", animation: "fadeUp 1s cubic-bezier(.22,1,.36,1) .5s both", margin: "0 0 52px" }}>
        ...
        <div style={{ width: 320, margin: "0 auto 20px", animation: "fadeIn .6s ease .7s both" }}>
          <div style={{ height: 2, background: "rgba(255,255,255,0.08)", borderRadius: 1, overflow: "hidden", marginBottom: 10 }}>
            <div style={{ height: "100%", width: `${progress}%`, background: `linear-gradient(90deg,${G[500]},${G[300]})`, borderRadius: 1, boxShadow: `0 0 12px ${G[400]}`, transition: "width .05s linear" }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "'Fira Code',monospace" }}>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: 11 }}>LOADING SYSTEMS</span>
            <span style={{ color: G[400], fontSize: 11, fontWeight: 600 }}>{Math.round(progress)}%</span>
          </div>
        </div>
        <div style={{ color: "rgba(255,255,255,0.25)", fontSize: 12, fontFamily: "'Fira Code',monospace", letterSpacing: ".1em", animation: "fadeIn .6s ease .9s both" }}>
          LAUNCHING IN <span style={{ color: G[500], fontWeight: 700 }}>{Math.max(counter, 0)}</span>S
        </div>
      </div>
      <div style={{ position: "absolute", bottom: 24, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 40, fontFamily: "'Fira Code',monospace", fontSize: 10, color: "rgba(34,197,94,0.3)", letterSpacing: ".1em", zIndex: 3 }}>
        {["GPT-4o ENGINE", "ATS CERTIFIED", "256-BIT SECURE", "99.9% UPTIME"].map(t => <span key={t}>{t}</span>)}
      </div>
    </div>
  );
}
...
function ThreeDBackground({ dark, scrollY }) {
  ...
  useEffect(() => {
    ...
    const onMouse = e => { mouseRef.current.vx = e.clientX - mouseRef.current.x; mouseRef.current.vy = e.clientY - mouseRef.current.y; mouseRef.current.x = e.clientX; mouseRef.current.y = e.clientY; };
    window.addEventListener("mousemove", onMouse);
    const NODES = Array.from({ length: 80 }, () => ({ x: (Math.random() - 0.5) * 2000, y: (Math.random() - 0.5) * 2000, z: Math.random() * 1200 + 100, vx: (Math.random() - 0.5) * 0.5, vy: (Math.random() - 0.5) * 0.4, vz: (Math.random() - 0.5) * 0.6, baseSize: Math.random() * 3 + 1, hue: 110 + Math.random() * 60, pulse: Math.random() * Math.PI * 2 }));
    const NEBULAS = Array.from({ length: 6 }, () => ({ x: Math.random() * 1.4 - 0.2, y: Math.random() * 1.4 - 0.2, r: Math.random() * 300 + 150, hue: Math.random() > 0.5 ? 130 : 200, phase: Math.random() * Math.PI * 2, speed: Math.random() * 0.003 + 0.001 }));
    const draw = () => {
      const S = stateRef.current; const M = mouseRef.current;
      S.time += 0.008;
      ctx.fillStyle = S.dark ? "rgba(2,7,3,0.14)" : "rgba(245,252,248,0.18)";
      ctx.fillRect(0, 0, W, H);
      NEBULAS.forEach(n => {
        n.phase += n.speed;
        const nx = n.x * W + Math.sin(n.phase) * 80; const ny = n.y * H + Math.cos(n.phase * 0.7) * 60;
      ...
      const scroll = S.scrollY; const gridAlpha = S.dark ? 0.025 : 0.012;
      const vpy = H * 0.5 - scroll * 0.1;
      ctx.save();
      for (let i = 0; i <= 16; i++) { const t = i / 16; const x = t * W; const pulse = Math.sin(S.time * 1.2 + t * Math.PI * 2) * 0.3 + 0.7; ctx.strokeStyle = `rgba(34,197,94,${gridAlpha * pulse})`; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(W / 2 + (x - W / 2) * 0.05, vpy); ctx.lineTo(x, H); ctx.stroke(); }
      for (let i = 0; i <= 10; i++) { const t = i / 10; const y = vpy + (H - vpy) * Math.pow(t, 1.5); const spread = 0.05 + t * 0.95; const pulse = Math.sin(S.time * 0.8 + t * 4) * 0.3 + 0.7; ctx.strokeStyle = `rgba(34,197,94,${gridAlpha * pulse})`; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(W / 2 - W / 2 * spread, y); ctx.lineTo(W / 2 + W / 2 * spread, y); ctx.stroke(); }
      ctx.restore();
      const projected = [];
      const fov = 600;
      NODES.forEach(n => {
        n.pulse += 0.02;
        n.x += n.vx + (M.x / (W || 1) - 0.5) * 0.4; n.y += n.vy + (M.y / (H || 1) - 0.5) * 0.4; n.z += n.vz;
        if (Math.abs(n.x) > 1200) n.vx *= -1; if (Math.abs(n.y) > 1200) n.vy *= -1; if (n.z < 80 || n.z > 1400) n.vz *= -1;
        const scale = fov / (fov + n.z);
        projected.push({ sx: n.x * scale + W / 2, sy: (n.y - scroll * 0.08) * scale + H / 2, scale, n });
      });
      for (let i = 0; i < projected.length; i++) {
        for (let j = i + 1; j < projected.length; j++) {
          ...
          if (dist < 120) {
            ...
            ctx.strokeStyle = grad2; ctx.lineWidth = avgScale * 0.7; ctx.stroke();
          }
        }
      }
      projected.forEach(({ sx, sy, scale, n }) => {
        ...
        if (r > 1.5) { ctx.beginPath(); ctx.arc(sx, sy, r * 2.5, 0, Math.PI * 2); ctx.fillStyle = `hsla(${n.hue},80%,55%,${alpha * 0.06})`; ctx.fill(); }
        ctx.beginPath(); ctx.arc(sx, sy, r, 0, Math.PI * 2);
        ctx.fillStyle = S.dark ? `hsla(${n.hue},85%,62%,${alpha})` : `hsla(${n.hue},70%,38%,${alpha * 0.55})`; ctx.fill();
      });
  ...
  return <canvas ref={canvasRef} style={{ position: "fixed", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 0, opacity: dark ? 0.75 : 0.35, transition: "opacity .6s" }} />;
}
...
function CustomCursor({ dark }) {
  ...
  return (
    <>
      ...
      <div ref={ring} style={{ position: "fixed", width: 36, height: 36, borderRadius: "50%", border: `2px solid ${c}`, pointerEvents: "none", zIndex: 9998, transform: "translate(-50%,-50%)", opacity: .6 }} />
    </>
  );
}
...
function FeatureCard({ f, i, dark }) {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setHovered(true)} onMouseLeave={() => { setHovered(false); setTilt({ x: 0, y: 0 }); }} onMouseMove={onMouseMove}
      style={{ background: bg, border: `1.5px solid ${border}`, borderRadius: 22, padding: "34px 28px", cursor: "default", position: "relative", overflow: "hidden", boxShadow: hovered ? `0 20px 60px ${f.glow}, 0 2px 8px rgba(0,0,0,0.12)` : "0 1px 4px rgba(0,0,0,0.04)", opacity: visible ? 1 : 0, transform: visible ? (hovered ? `translateY(-10px) scale(1.02) perspective(600px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)` : "translateY(0) scale(1)") : "translateY(48px) scale(0.95)", transition: `opacity .6s cubic-bezier(.22,1,.36,1) ${i * 100}ms, transform .45s cubic-bezier(.22,1,.36,1), background .3s, border-color .3s, box-shadow .3s` }}>
      <div style={{ width: 58, height: 58, borderRadius: 18, background: `${f.color}18`, border: `1.5px solid ${f.color}40`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, marginBottom: 22, transform: hovered ? "rotate(-10deg) scale(1.15)" : "rotate(0) scale(1)", transition: "transform .35s cubic-bezier(.34,1.56,.64,1)", boxShadow: hovered ? `0 8px 24px ${f.glow}` : "none" }}>
        {f.icon}
      </div>
      <h3 style={{ margin: "0 0 10px", fontSize: 17, fontWeight: 700, color: dark ? "#f1f5f9" : "#111827", letterSpacing: "-0.02em" }}>
        <SplitText text={f.title} stagger={22} delay={i * 80} type="slideUp" />
      </h3>
...
function TestiCard({ t, i, dark }) {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: dark ? (hovered ? "#1e293b" : "#0f172a") : "#fff", border: `1.5px solid ${hovered ? (dark ? "#22c55e55" : G[300]) : (dark ? "#1e293b" : G[100])}`, borderRadius: 22, padding: "28px 24px", boxShadow: hovered ? `0 16px 40px rgba(34,197,94,0.15)` : "0 2px 12px rgba(0,0,0,0.05)", transform: hovered ? "translateY(-6px) scale(1.01)" : (visible ? "translateY(0)" : "translateY(44px)"), opacity: visible ? 1 : 0, transition: `opacity .65s ease ${i * 110}ms, transform .4s cubic-bezier(.22,1,.36,1), box-shadow .3s` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 42, height: 42, borderRadius: "50%", background: dark ? "#1e3a2f" : G[50], border: `1.5px solid ${dark ? "#22c55e33" : G[200]}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, transform: hovered ? "scale(1.1) rotate(-5deg)" : "scale(1)", transition: "transform .3s cubic-bezier(.34,1.56,.64,1)" }}>{t.emoji}</div>
        <div>
...
function PricingCard({ p, i, dark }) {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: p.popular ? `linear-gradient(160deg,${G[600]},${G[700]})` : (dark ? "#0f172a" : "#fff"), border: p.popular ? "none" : `1.5px solid ${hovered ? p.color + "55" : (dark ? "#1e293b" : "#e5e7eb")}`, borderRadius: 24, padding: "36px 28px", position: "relative", boxShadow: hovered ? (p.popular ? `0 24px 64px rgba(34,197,94,0.5)` : `0 16px 48px ${p.color}30`) : (p.popular ? `0 16px 48px rgba(34,197,94,0.35)` : "0 2px 12px rgba(0,0,0,0.05)"), transform: visible ? (hovered ? "translateY(-10px) scale(1.02)" : (p.popular ? "translateY(-6px)" : "translateY(0)")) : "translateY(40px)", opacity: visible ? 1 : 0, transition: `opacity .6s ease ${i * 130}ms, transform .4s cubic-bezier(.22,1,.36,1), box-shadow .3s` }}>
      {p.popular && (<div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(90deg,#fbbf24,#f59e0b)", borderRadius: 999, padding: "6px 20px", fontSize: 12, fontWeight: 800, color: "#78350f", boxShadow: "0 4px 16px rgba(245,158,11,0.5)", whiteSpace: "nowrap" }}>✦ Most Popular</div>)}
      ...
      <button style={{ width: "100%", padding: "15px", borderRadius: 14, border: "none", cursor: "pointer", background: p.popular ? "rgba(255,255,255,0.2)" : p.color, color: "#fff", fontSize: 15, fontWeight: 700, fontFamily: "inherit", transition: "all .3s", boxShadow: hovered ? `0 8px 24px ${p.popular ? "rgba(255,255,255,0.2)" : p.color + "60"}` : "none", transform: hovered ? "scale(1.02)" : "scale(1)" }}>{p.cta} →</button>
    </div>
  );
}
...
function TemplatePreview({ t, dark }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ borderRadius: 20, overflow: "hidden", position: "relative", cursor: "pointer", transform: hovered ? "translateY(-8px) scale(1.02)" : "translateY(0) scale(1)", boxShadow: hovered ? `0 24px 56px rgba(0,0,0,0.25)` : "0 8px 24px rgba(0,0,0,0.12)", transition: "all .35s cubic-bezier(.22,1,.36,1)" }}>
      <div style={{ background: t.color, height: 220, padding: 20, position: "relative" }}>
        ...
        {hovered && <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ background: t.accent, color: "#fff", padding: "10px 24px", borderRadius: 999, fontWeight: 700, fontSize: 14 }}>Use Template →</div></div>}
      </div>
  ...
  return (
    <div ref={ref} style={{ borderBottom: `1px solid ${dark ? "#1e293b" : "#e5e7eb"}`, opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(28px)", transition: `all .6s cubic-bezier(.22,1,.36,1) ${i * 100}ms` }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: "100%", padding: "22px 0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 16, color: dark ? "#f1f5f9" : "#111827", textAlign: "left" }}>
        <SplitText text={q} stagger={18} delay={i * 60} type="slideUp" style={{ display: "block" }} />
...
function ThemeToggle({ dark, setDark }) {
  return (
    <button onClick={() => setDark(d => !d)} style={{ width: 52, height: 28, borderRadius: 999, border: `2px solid ${dark ? "#22c55e33" : "#e5e7eb"}`, background: dark ? "#1e293b" : "#f9fafb", cursor: "pointer", position: "relative", transition: "all .3s", flexShrink: 0 }}>
      <div style={{ position: "absolute", top: 2, left: dark ? 26 : 2, width: 20, height: 20, borderRadius: "50%", background: dark ? G[400] : "#9ca3af", transition: "all .3s cubic-bezier(.34,1.56,.64,1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11 }}>
        {dark ? "🌙" : "☀️"}
      </div>
    </button>
  );
...
function SectionLabel({ text, dark }) {
  return (
    <div style={{ fontSize: 12, color: G[600], fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", marginBottom: 12 }}>
      <SplitText text={text} stagger={30} delay={0} type="fadeScale" />
    </div>
  );
}
...
export default function Home() {
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes letterBounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}
    @keyframes charGlow{0%,100%{text-shadow:0 0 8px rgba(34,197,94,0.2)}50%{text-shadow:0 0 20px rgba(34,197,94,0.7)}}
    ...
    .hover-letter-wrap span[data-letter]:hover{
      color:${G[400]};
      transform:translateY(-6px) scale(1.15);
      transition:transform .15s cubic-bezier(.34,1.56,.64,1), color .15s !important;
    }
    .gradient-text-animated{
      background: linear-gradient(120deg, ${G[600]}, ${G[400]}, #38bdf8, ${G[500]});
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans','Segoe UI',sans-serif", background: bg, color: text, overflowX: "hidden", transition: "background .3s,color .3s", position: "relative" }}>
      ...
      {/* ─── NAV ─── */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 200, padding: "0 5%", height: 68, background: scrolled ? navBg : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", borderBottom: scrolled ? `1px solid ${navBorder}` : "1px solid transparent", boxShadow: scrolled ? (dark ? "0 2px 24px rgba(0,0,0,.4)" : "0 2px 24px rgba(0,0,0,.07)") : "none", transition: "all .4s ease", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", zIndex: 1 }}>
          ...
          <button className="btn-green" style={{ padding: "9px 20px", fontSize: 14 }}>Get Started Free →</button>
        </div>
      </nav>

      {/* ─── HERO ─── */}
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "110px 5% 80px", position: "relative", overflow: "hidden", background: dark ? `radial-gradient(ellipse 90% 70% at 50% -5%,#0d2818 0%,${bg} 65%)` : `radial-gradient(ellipse 90% 70% at 50% -5%,${G[50]} 0%,#fff 65%)` }}>
        {[700, 500, 320].map((size, i) => (
          <div key={i} style={{ position: "absolute", width: size, height: size, borderRadius: "50%", border: `1px ${i === 1 ? "dashed" : "solid"} ${dark ? (i === 2 ? "#22c55e33" : "#1e293b") : (i === 2 ? G[300] : G[200])}`, top: "50%", left: "50%", transform: "translate(-50%,-50%)", pointerEvents: "none", opacity: .6, animation: `spinSlow ${30 + i * 12}s linear ${i % 2 === 0 ? "" : "reverse"} infinite` }} />
        ))}
        ...
        <FloatBadge style={{ left: "5%", bottom: "20%", width: 180, opacity: .9 }} delay={1.5} dark={dark}>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, color: dark ? "#e2e8f0" : "#374151" }}>Akash got hired at Stripe</div>
        ...
        <div style={{ maxWidth: 760, textAlign: "center", position: "relative", zIndex: 2, transform: `translateY(${-heroParallax * 0.2}px)` }}>
          ...
          {/* ── HERO HEADLINE with per-char animation ── */}
          <h1 style={{ fontSize: "clamp(44px,6.5vw,80px)", fontWeight: 800, lineHeight: 1.06, letterSpacing: "-0.045em", margin: "0 0 12px", fontFamily: "'Lora','Georgia',serif", color: text, display: "block" }}>
            <span style={{ display: "block", overflow: "hidden" }}>
              {"Build a ".split("").map((char, i) => (
                <span key={i} style={{
                  ...
                  animation: `fadeUp .9s cubic-bezier(.22,1,.36,1) ${i * 35}ms both`,
                  whiteSpace: char === " " ? "pre" : undefined,
                }}>{char === " " ? "\u00A0" : char}</span>
              ))}
            </span>
            ...
            <span style={{ display: "block", overflow: "hidden" }}>
              {"Resume in Minutes".split("").map((char, i) => (
                <span key={i} style={{
                  display: "inline-block",
                  animation: `fadeUp .9s cubic-bezier(.22,1,.36,1) ${300 + i * 35}ms both`,
                  whiteSpace: char === " " ? "pre" : undefined,
                }}>{char === " " ? "\u00A0" : char}</span>
              ))}
            </span>
          ...
          <p style={{ fontSize: 18.5, color: muted, lineHeight: 1.68, maxWidth: 540, margin: "16px auto 40px", animation: "fadeUp .9s cubic-bezier(.22,1,.36,1) .7s both" }}>
            AI that understands your career. ATS-beating templates. Your dream job — one click away ❤️
          </p>

          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", animation: "fadeUp .9s cubic-bezier(.22,1,.36,1) .85s both" }}>
            <button className="btn-green" style={{ padding: "18px 40px", fontSize: 16 }}>Build My Resume — Free 🎉</button>
          ...
          <p style={{ marginTop: 18, fontSize: 13, color: dark ? "#475569" : "#9ca3af", animation: "fadeIn 1s ease 1s both" }}>
            No credit card · Free forever plan · 3-minute setup
          </p>

          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginTop: 28, animation: "fadeUp .9s ease 1.1s both" }}>
            <div style={{ display: "flex" }}>
              {["🧑‍💼", "👩‍💻", "🧑‍🎨", "👩‍🔬", "🧑‍💻", "👩‍🎤"].map((e, i) => (
                <div key={i} style={{ width: 36, height: 36, borderRadius: "50%", background: `hsl(${135 + i * 18},55%,${dark ? "25%" : "88%"})`, border: `2.5px solid ${bg}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, marginLeft: i === 0 ? 0 : -12, zIndex: 10 - i, animation: `fadeIn .4s ease ${i * 80}ms both` }}>{e}</div>
              ))}
            </div>
          ...
          <div style={{ marginTop: 48, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, animation: "fadeIn 1.2s ease 1.3s both", opacity: .5 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 0, border: `1px solid ${statsBorder}`, borderRadius: 24, overflow: "hidden", boxShadow: dark ? "0 4px 24px rgba(0,0,0,0.3)" : "0 4px 24px rgba(0,0,0,0.05)" }}>
          {STATS.map((s, i) => (
            <div key={i} style={{ textAlign: "center", padding: "48px 24px", borderLeft: i > 0 ? `1px solid ${statsBorder}` : "none", background: statsV ? statsCardBg : (dark ? "#0a0f1a" : G[50]), transition: `background .5s ease ${i * 100}ms`, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ fontSize: 14, color: muted, fontWeight: 500, position: "relative", opacity: statsV ? 1 : 0, transition: `opacity .6s ease ${i * 120 + 200}ms` }}>
                <SplitText text={s.label} stagger={40} delay={i * 120 + 300} type="fadeScale" />
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="features-section" style={{ padding: "60px 5% 100px", background: surface, position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <SectionLabel text="Why AIResume" dark={dark} />
            <div style={{ fontSize: "clamp(30px,4vw,52px)", fontWeight: 800, letterSpacing: "-0.04em", margin: "0 0 14px", fontFamily: "'Lora',serif", color: text, lineHeight: 1.12 }}>
              <SplitWords text="Everything you need" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(30px,4vw,52px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={80} type="slideUp" />
              <SplitWords text="to get hired faster" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(30px,4vw,52px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: G[500] }} stagger={80} delay={300} type="slideDown" />
            </div>
      ...
      <section id="templates-section" style={{ padding: "80px 5% 100px", background: bg, position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <SectionLabel text="Templates" dark={dark} />
            <div style={{ marginBottom: 14, lineHeight: 1.12 }}>
              <SplitWords text="Recruiter-approved designs" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={90} type="slideUp" />
            </div>
            <Reveal direction="up" delay={300}>
              <p style={{ color: muted, fontSize: 16, maxWidth: 440, margin: "0 auto 32px" }}>Every template is crafted to pass ATS and impress hiring managers.</p>
      ...
      <section ref={stepsRef} style={{ padding: "100px 5%", maxWidth: 1040, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 72 }}>
          ...
          <SplitWords text="Three steps to your dream job" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(28px,4vw,48px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={70} type="slideUp" />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, position: "relative" }}>
          ...
          {STEPS.map((s, i) => (
            <div key={i} style={{ textAlign: "center", padding: "0 28px", position: "relative", zIndex: 1, opacity: stepsV ? 1 : 0, transform: stepsV ? "translateY(0)" : "translateY(44px)", transition: `all .8s cubic-bezier(.22,1,.36,1) ${i * 180}ms` }}>
              ...
              <h3 style={{ fontSize: 20, fontWeight: 800, margin: "0 0 10px", letterSpacing: "-0.025em", color: text }}>
                <SplitText text={s.title} stagger={30} delay={i * 200 + 400} type="rotateIn" />
              </h3>
              <p style={{ color: muted, fontSize: 14.5, lineHeight: 1.68 }}>
                <SplitText text={s.desc} stagger={12} delay={i * 200 + 600} type="blur" wordByWord={true} />
              </p>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "80px 5% 100px", background: surface, position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 1120, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <Reveal direction="left" delay={0}>
            <div>
              <SectionLabel text="Live Editor" dark={dark} />
              <div style={{ marginBottom: 18, lineHeight: 1.16 }}>
                <SplitWords text="Watch your resume come alive as you type" style={{ flexWrap: "wrap" }} wordStyle={{ fontSize: "clamp(26px,3.2vw,42px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={60} type="slideUp" />
              </div>
              <Reveal direction="up" delay={400}>
                ...
                {["Smart autofill from your LinkedIn profile", "Instant ATS compatibility score live", "One-click tone: formal, friendly, bold", "Auto-translated for 40+ languages"].map((item, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 15, color: dark ? "#cbd5e1" : "#374151", marginBottom: 14 }}>
                    ...
                    <SplitText text={item} stagger={15} delay={i * 120 + 300} type="slideUp" />
                  </div>
                ))}
                <button className="btn-green" style={{ marginTop: 16, padding: "15px 32px", fontSize: 15 }}>Try the Editor Free →</button>
              </Reveal>
            </div>
          </Reveal>

          <Reveal direction="right" delay={100}>
            <div style={{ background: dark ? "#0f172a" : "#fff", border: `1.5px solid ${dark ? "#1e293b" : G[200]}`, borderRadius: 28, padding: 30, boxShadow: dark ? `0 24px 64px rgba(0,0,0,0.5)` : `0 24px 64px rgba(0,0,0,0.08)`, position: "relative", animation: "float 7s ease-in-out infinite" }}>
      ...
      <section style={{ padding: "100px 5%", maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 60 }}>
          <SectionLabel text="Testimonials" dark={dark} />
          <SplitWords text="Loved by job seekers worldwide" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(26px,3.5vw,46px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={80} type="dropIn" />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18 }}>
      ...
      <section id="pricing-section" style={{ padding: "80px 5% 100px", background: surface, position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 1060, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <SplitWords text="Simple, transparent pricing" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(28px,4vw,50px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={80} type="slideUp" />
            <Reveal direction="up" delay={300}>
              <p style={{ color: muted, fontSize: 16, maxWidth: 440, margin: "14px auto 0" }}>Start free. Upgrade when you're ready. Cancel anytime.</p>
            </Reveal>
          </div>
      ...
      <section id="about-section" style={{ padding: "80px 5%", maxWidth: 760, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          <SplitWords text="Frequently asked questions" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(24px,3.5vw,40px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: text }} stagger={60} type="flipX" />
        </div>
        {[
        ...
        ].map((item, i) => <FAQItem key={i} q={item.q} a={item.a} i={i} dark={dark} />)}
      </section>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", background: `linear-gradient(135deg,${G[500]},${G[600]},${G[700]})`, borderRadius: 36, padding: "76px 52px", animation: "glowPulse 3s ease-in-out infinite", opacity: ctaV ? 1 : 0, transform: ctaV ? "scale(1) translateY(0)" : "scale(0.92) translateY(32px)", transition: "opacity .8s cubic-bezier(.22,1,.36,1), transform .8s cubic-bezier(.22,1,.36,1)", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "relative", zIndex: 2 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <SplitWords text="Your next job starts here" style={{ justifyContent: "center" }} wordStyle={{ fontSize: "clamp(28px,4vw,46px)", fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "'Lora',serif", color: "#fff" }} stagger={80} type="slideUp" />
            </div>
            ...
            <button className="cta-white-btn">Create My Free Resume →</button>
      ...
      <footer style={{ background: dark ? "linear-gradient(160deg,#000000,#0a0f1a 40%,#0d2818)" : "linear-gradient(160deg,#0f172a,#111827 40%,#14532d)", padding: "70px 5% 25px", position: "relative", zIndex: 1, overflow: "hidden" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 40, position: "relative", zIndex: 2, marginBottom: 50 }}>
          <div>
            ...
            <p style={{ color: "rgba(255,255,255,0.65)", lineHeight: 1.8, maxWidth: 320, fontSize: 14 }}>Build AI-powered resumes that stand out, beat ATS systems, and help you land your dream job faster.</p>
            <div style={{ display: "flex", gap: 14, marginTop: 24 }}>
              {["🌐", "📘", "📸", "💼"].map((icon, i) => (<div key={i} className="social-icon-btn">{icon}</div>))}
            </div>
          </div>
          <div>
            <h3 style={{ color: "#fff", marginBottom: 18, fontSize: 16, fontWeight: 600 }}>Product</h3>
            {["Features", "Templates", "Pricing", "Builder", "Changelog"].map(item => (<p key={item} className="footer-link-item"><span style={{ fontSize: 10, opacity: .5 }}>→</span> {item}</p>))}
          </div>
          <div>
            <h3 style={{ color: "#fff", marginBottom: 18, fontSize: 16, fontWeight: 600 }}>Company</h3>
            {["About", "Careers", "Blog", "Press", "Contact"].map(item => (<p key={item} className="footer-link-item"><span style={{ fontSize: 10, opacity: .5 }}>→</span> {item}</p>))}
          </div>
          <div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 22, display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: 24 }}>
            {["Privacy Policy", "Terms", "Support"].map(item => (<span key={item} className="footer-legal-item">{item}</span>))}
          </div>
        </div>
      </footer>
    </div>
