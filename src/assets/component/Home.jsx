...
function Particles({count=35,color="34,197,94"}){
  const ref=useRef();const rafRef=useRef();const ptsRef=useRef([]);const mouseRef=useRef({x:-999,y:-999});
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      ptsRef.current.forEach(p=>{
        const dx=p.x-m.x;const dy=p.y-m.y;const dist=Math.hypot(dx,dy);
        if(dist<100){const force=(100-dist)/100*0.8;p.vx+=(dx/dist)*force;p.vy+=(dy/dist)*force;}
      ...
      ptsRef.current.forEach((a,i)=>{
        ptsRef.current.slice(i+1).forEach(b=>{
          ...
          if(d<130){ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle=`rgba(${color},${.15*(1-d/130)})`;ctx.lineWidth=.8;ctx.stroke();}
        });
      });
...
function CustomCursor({dark}){
  ...
  return(
    <>
      <div ref={dot} style={{position:"fixed",width:8,height:8,borderRadius:"50%",background:c,pointerEvents:"none",zIndex:9999,transform:"translate(-50%,-50%)"}}/>
...
function FeatureCard({f,i,dark}){
  ...
  return(
    <div ref={ref} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>{setHovered(false);setTilt({x:0,y:0});}} onMouseMove={onMouseMove}
      ...
      {hovered&&<div style={{position:"absolute",inset:0,borderRadius:22,pointerEvents:"none",background:"linear-gradient(135deg, rgba(255,255,255,0.05) 0%, transparent 50%)"}}/>}
    </div>
  );
}
    ...
    <div ref={ref} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div style={{display:"flex",gap:3,marginBottom:16}}>
        {[...Array(t.rating)].map((_,j)=>(
          <span key={j} style={{color:G[400],fontSize:16,transform:`scale(${hovered?1.2:1}) rotate(${hovered?(j-2)*5:0}deg)`,transition:`transform .3s ease ${j*50}ms`,display:"inline-block"}}>★</span>
        ))}
      </div>
...
function PricingCard({p,i,dark}){
  ...
  return(
    <div ref={ref} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <button style={{width:"100%",padding:"15px",borderRadius:14,border:"none",cursor:"pointer",background:p.popular?"rgba(255,255,255,0.2)":p.color,color:"#fff",fontSize:15,fontWeight:700,fontFamily:"inherit",letterSpacing:"-0.01em",transition:"all .3s cubic-bezier(.22,1,.36,1)",boxShadow:hovered?`0 8px 24px ${p.popular?"rgba(255,255,255,0.2)":p.color+"60"}`:"none",transform:hovered?"scale(1.02)":"scale(1)"}}>
        {p.cta} →
      </button>
    </div>
  );
...
function Reveal({children,delay=0,direction="up",className=""}){
  ...
  return(
    <div ref={ref} className={className} style={{opacity:visible?1:0,transform:visible?"translate(0,0)":transforms[direction],transition:`opacity .7s cubic-bezier(.22,1,.36,1) ${delay}ms, transform .7s cubic-bezier(.22,1,.36,1) ${delay}ms`}}>
      {children}
    </div>
  );
}
...
function FAQItem({q,a,i,dark}){
  ...
  return(
    <div ref={ref} style={{borderBottom:`1px solid ${dark?"#1e293b":"#e5e7eb"}`,marginBottom:0,opacity:visible?1:0,transform:visible?"translateY(0)":"translateY(28px)",transition:`all .6s cubic-bezier(.22,1,.36,1) ${i*100}ms`}}>
    ...
    <div onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div style={{background:t.color,height:220,padding:20,position:"relative"}}>
        <div style={{background:"rgba(255,255,255,0.08)",borderRadius:12,padding:14,height:"100%",display:"flex",flexDirection:"column",gap:8}}>
          ...
          {[65,50].map((w,i)=><div key={i} style={{height:5,width:`${w}%`,background:"rgba(255,255,255,0.2)",borderRadius:3}}/>)}
        </div>
        {hovered&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .3s"}}>
          <div style={{background:t.accent,color:"#fff",padding:"10px 24px",borderRadius:999,fontWeight:700,fontSize:14}}>Use Template →</div>
        </div>}
      </div>
 ...
 const { dark, setDark } = useTheme();
  ...
  const css=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes badgeIn{from{opacity:0;transform:translateY(-10px) scale(0.9)}to{opacity:1;transform:translateY(0) scale(1)}}
    ...
    .nav-link{color:${dark?"#94a3b8":"#4b5563"};text-decoration:none;font-size:14px;font-weight:500;transition:all .2s;position:relative}
  ...
  return(
    <div style={{fontFamily:"'Plus Jakarta Sans','Segoe UI',sans-serif",background:bg,color:text,overflowX:"hidden",transition:"background .3s,color .3s"}}>
      <style>{css}</style>
      <ScrollProgress/>
      ...
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,padding:"0 5%",height:68,
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <button className="btn-outline" style={{padding:"9px 20px",fontSize:14}} onClick={()=>navigate("/login")}>Log in</button>
      ...
      <div style={{overflow:"hidden",borderTop:`1px solid ${dark?"#1e293b":G[100]}`,borderBottom:`1px solid ${dark?"#1e293b":G[100]}`,padding:"14px 0",background:dark?"#0a0f1a":G[50]}}>
      ...
      <section style={{padding:"60px 5% 100px",background:surface}}>
      ...
      <section style={{padding:"80px 5% 100px",background:bg}}>
      ...
      <section style={{padding:"80px 5% 100px",background:surface}}>
        <div style={{maxWidth:1120,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"center"}}>
          <Reveal direction="left" delay={0}>
            ...
            {["Smart autofill from your LinkedIn profile","Instant ATS compatibility score live","One-click tone: formal, friendly, bold","Auto-translated for 40+ languages"].map((item,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:12,fontSize:15,color:dark?"#cbd5e1":"#374151",marginBottom:14,animation:`slideInLeft .6s cubic-bezier(.22,1,.36,1) ${i*100+200}ms both`}}>
      ...
      <footer style={{background:dark?"linear-gradient(160deg,#000000,#0a0f1a 40%,#0d2818)":"linear-gradient(160deg,#0f172a,#111827 40%,#14532d)",padding:"70px 5% 25px",position:"relative",overflow:"hidden"}}>
